...
describe('[PATCH]: /api/urls/:id/like', () => {
  const client = treaty(app);

  it('should like a public URL when authenticated', async () => {
  ...
  it('should return 400 when URL is already liked', async () => {
    const { sessionHeaders: creatorHeaders } = await createAuthenticatedUser();
    const { sessionHeaders: likerHeaders } = await createAuthenticatedUser();
  ...
  it('should return 405 when trying to like a private URL', async () => {
    const { sessionHeaders: creatorHeaders } = await createAuthenticatedUser();
    const { sessionHeaders: likerHeaders } = await createAuthenticatedUser();
